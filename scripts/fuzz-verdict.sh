...
#
# cancelCtx.cancel stores the error, closes its own done channel, and only then
# cancels its children, and cancelCtx.Err() is a lock-free atomic load. So a
# coordinator woken between those two steps reads fuzzCtx.Err() as nil, fails
# to suppress, and returns context.DeadlineExceeded as the target's result. The
# target ran its whole budget and found nothing; the run reports FAIL anyway.
#
# Upstream: golang/go#75804, fixed by https://go.dev/cl/774140. Not in any
# 1.26.x. Delete this script and its callers once the toolchain this repository
# pins has the fix.
#
...
# which fails before any input is written, is named separately. A hung worker
# (golang/go#56238) is a different bug this script has no evidence about, so it
# stays a failure.
#
